...
with gr.Blocks(title="AI 虚拟试衣", css="footer{display:none !important}") as demo:
    ...
    with gr.Row():
        person_input = gr.Image(label="人物照片", type="pil", sources=["upload"], height=260)
        cloth_input = gr.Image(label="衣服图片", type="pil", sources=["upload"], height=260)
    ...
    with gr.Row():
        result_output = gr.Image(label="试穿效果", type="pil", height=580)
