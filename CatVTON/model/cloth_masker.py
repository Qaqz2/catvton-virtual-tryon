...
DENSE_INDEX_MAP = {
    "background": [0], "torso": [1, 2], "right hand": [3], "left hand": [4],
    "right foot": [5], "left foot": [6], "right thigh": [7, 9], "left thigh": [8, 10],
    "right leg": [11, 13], "left leg": [12, 14], "left big arm": [15, 17],
    "right big arm": [16, 18], "left forearm": [19, 21], "right forearm": [20, 22],
    "face": [23, 24], "thighs": [7, 8, 9, 10], "legs": [11, 12, 13, 14],
    "hands": [3, 4], "feet": [5, 6], "big arms": [15, 16, 17, 18], "forearms": [19, 20, 21, 22],
}
...
PROTECT_BODY_PARTS = {
    'upper': ['Left-leg', 'Right-leg'], 'lower': ['Right-arm', 'Left-arm', 'Face'],
    'overall': [], 'inner': ['Left-leg', 'Right-leg'], 'outer': ['Left-leg', 'Right-leg'],
}
PROTECT_CLOTH_PARTS = {
...
MASK_DENSE_PARTS = {
    'upper': ['torso', 'big arms', 'forearms'], 'lower': ['thighs', 'legs'],
    'overall': ['torso', 'thighs', 'legs', 'big arms', 'forearms'],
    'inner': ['torso'], 'outer': ['torso', 'big arms', 'forearms']
}
...
def part_mask_of(part, parse, mapping):
...
def hull_mask(mask_area):
    ret, binary = cv2.threshold(mask_area.astype(np.uint8), 127, 255, cv2.THRESH_BINARY)
    contours, hierarchy = cv2.findContours(binary, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    hull_mask_result = np.zeros_like(mask_area)
    for c in contours:
        hull = cv2.convexHull(c)
        hull_mask_result = cv2.fillPoly(np.zeros_like(mask_area), [hull], 255) | hull_mask_result
    return hull_mask_result


class AutoMasker:
    def __init__(self, densepose_ckpt='./Models/DensePose', schp_ckpt='./Models/SCHP', device='cuda'):
        np.random.seed(0)
        torch.manual_seed(0)
    ...
    @staticmethod
    def cloth_agnostic_mask(densepose_mask, schp_lip_mask, schp_atr_mask, part='overall', **kwargs):
        ...
        hair_protect_area = part_mask_of(['Hair'], schp_lip_mask, LIP_MAPPING) | part_mask_of(['Hair'], schp_atr_mask, ATR_MAPPING)
        cloth_protect_area = part_mask_of(PROTECT_CLOTH_PARTS[part]['LIP'], schp_lip_mask, LIP_MAPPING) | part_mask_of(PROTECT_CLOTH_PARTS[part]['ATR'], schp_atr_mask, ATR_MAPPING)
        accessory_parts = ['Hat', 'Glove', 'Sunglasses', 'Bag', 'Left-shoe', 'Right-shoe', 'Scarf', 'Socks']
        accessory_protect_area = part_mask_of(accessory_parts, schp_lip_mask, LIP_MAPPING) | part_mask_of(accessory_parts, schp_atr_mask, ATR_MAPPING)
        weak_protect_area = body_protect_area | cloth_protect_area | hair_protect_area | strong_protect_area | accessory_protect_area

        strong_mask_area = part_mask_of(MASK_CLOTH_PARTS[part], schp_lip_mask, LIP_MAPPING) | part_mask_of(MASK_CLOTH_PARTS[part], schp_atr_mask, ATR_MAPPING)
        ...
        mask_area = cv2.GaussianBlur((mask_area * 255).astype(np.float32), (kernal_size, kernal_size), 0)
        mask_area[mask_area < 25] = 0
        mask_area[mask_area >= 25] = 1
        mask_area = (mask_area.astype(bool) | strong_mask_area) & (~strong_protect_area)
