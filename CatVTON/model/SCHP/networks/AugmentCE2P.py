...
@File    :   AugmentCE2P.py
@Time    :   8/4/19 3:35 PM
@Desc    :   CE2P (Context Embedding with Edge Perceiving) for Human Parsing
...
class PSPModule(nn.Module):
    ...
    def forward(self, feats):
        ...
        priors = [
            F.interpolate(input=stage(feats), size=(h, w),
...
class ASPPModule(nn.Module):
    ...
    def __init__(self, features, inner_features=256, out_features=512,
        ...
        self.bottleneck = nn.Sequential(
            nn.Conv2d(inner_features * 5, out_features, kernel_size=1,
    ...
    def forward(self, x):
        ...
        feat1 = F.interpolate(self.conv1(x), size=(h, w),
                              mode='bilinear', align_corners=True)
...
class Edge_Module(nn.Module):
    ...
    def __init__(self, in_fea=[256, 512, 1024], mid_fea=256, out_fea=2):
        ...
        self.conv5 = nn.Conv2d(out_fea * 3, out_fea, kernel_size=1, padding=0,
                               dilation=1, bias=True)
    ...
    def forward(self, x1, x2, x3):
        ...
        edge2_fea = F.interpolate(edge2_fea, size=(h, w),
                                  mode='bilinear', align_corners=True)
        edge3_fea = F.interpolate(edge3_fea, size=(h, w),
                                  mode='bilinear', align_corners=True)
        edge2 = F.interpolate(edge2, size=(h, w),
                              mode='bilinear', align_corners=True)
        edge3 = F.interpolate(edge3, size=(h, w),
                              mode='bilinear', align_corners=True)
...
class Decoder_Module(nn.Module):
    ...
    def __init__(self, num_classes):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(512, 256, kernel_size=1, padding=0, dilation=1,
                      bias=False),
            InPlaceABNSync(256)
        )
        self.conv2 = nn.Sequential(
            nn.Conv2d(256, 48, kernel_size=1, stride=1, padding=0,
                      dilation=1, bias=False),
            InPlaceABNSync(48)
        )
        self.conv3 = nn.Sequential(
            nn.Conv2d(304, 256, kernel_size=1, padding=0, dilation=1,
                      bias=False),
            InPlaceABNSync(256),
    ...
    def forward(self, xt, xl):
        _, _, h, w = xl.size()
        xt = F.interpolate(self.conv1(xt), size=(h, w),
                           mode='bilinear', align_corners=True)
        xl = self.conv2(xl)
...
class ResNet(nn.Module):
    def __init__(self, block, layers, num_classes):
        ...
        self.fushion = nn.Sequential(
            nn.Conv2d(1024, 256, kernel_size=1, padding=0, dilation=1,
                      bias=False),
            InPlaceABNSync(256),
            nn.Dropout2d(0.1),
            nn.Conv2d(256, num_classes, kernel_size=1, padding=0, dilation=1,
                      bias=True)
        )
...
def initialize_pretrained_model(model, settings,
                                pretrained='./models/resnet101-imagenet.pth'):
